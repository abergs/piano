...
const STOP_INTERSECT_DISTANCE = 10;
...
const client = new Paho.MQTT.Client('wss://dev.hsl.fi/mqtt-proxy', clientId);
client.onMessageArrived = message =>
  incomingMessages.push(JSON.parse(message.payloadString));
client.connect({ onSuccess: () => client.subscribe('/hfp/journey/tram/#') });

// Consume the queue following the current rhythm pattern
function dequeNext(time) {
  if (incomingMessages.length) {
    processMessage(time, incomingMessages.shift());
  }
...
function processMessage(time, { VP: { desi, lat, long, tsi, veh } }) {
